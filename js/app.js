...
function calcularPago(d) {
  ...
  const horasDisponiblesTrasDescuentos = Math.max(0, num(d.hOrdinarias) - horasPermiso - horasAusencia);
  const horasIncapacidadPagadas = Math.min(horasDisponiblesTrasDescuentos, horasIncapacidad);
  const horasOrdinariasPagadas = Math.max(0, horasDisponiblesTrasDescuentos - horasIncapacidadPagadas);
  const horasSeptimoPagadas = diasSinPermiso > 0 ? 0 : num(d.hSeptimo);
  const ord = horasOrdinariasPagadas * salario;
  const incapacidad = horasIncapacidadPagadas * salario;
  ...
  const septimo = horasSeptimoPagadas * salario;
  ...
  return {
    ...
    horasOrdinariasPagadas: red(horasOrdinariasPagadas), horasIncapacidadPagadas: red(horasIncapacidadPagadas), horasSeptimoPagadas: red(horasSeptimoPagadas),
    descuentoPermiso: red(Math.min(num(d.hOrdinarias), horasPermiso) * salario),
...
function totalesDetallePlanilla(planillas) {
  return planillas.reduce((total, p) => {
    const c = p.calc || {};
    total.horasD += num(c.horasOrdinariasPagadas ?? p.hOrdinarias);
    ...
    total.horasSeptimo += num(c.horasSeptimoPagadas ?? p.hSeptimo);
    ...
    total.horasIncapacidad += num(c.horasIncapacidadPagadas);
    ...
    return total;
  }, { horasD: 0, extraD: 0, extraN: 0, domingoD: 0, domingoN: 0, horasSeptimo: 0, horasAsueto: 0, asuetoExtraD: 0, asuetoExtraN: 0, horasPermiso: 0, diasFalta: 0, horasIncapacidad: 0, devengado: 0, renta: 0, isss: 0, afp: 0, otros: 0, neto: 0 });
}
...
function generarCopiaBoleta(p, boleta, vacia = false) {
  ...
  const asistenciaNota = !vacia && (num(p?.hPermiso) > 0 || num(p?.diasSinPermiso) > 0)
    ? `<div class="paper-line paper-note"><span class="label">Permiso: ${num(p.hPermiso).toFixed(2)} h · Faltas: ${num(p.diasSinPermiso).toFixed(0)} d${num(p.diasSinPermiso) > 0 ? ' · sin séptimo' : ''}</span></div>`
    : '';
          ...
          <div class="receipt-columns">
            <div class="receipt-income-col">
              ...
              <div>
                ...
                ${line('H. Ordi. Diurnas:', c.horasOrdinariasPagadas ?? p?.hOrdinarias, c.ord)}
                ...
                ${asistenciaNota}
                ${line('H. Incapacidad:', c.horasIncapacidadPagadas, c.incapacidad)}
...
function renderGananciasMenusuales() {
  ...
  state.historialPagos.forEach(pago => {
    ...
    if (anioPago === anio) {
      ...
      if (!gananciasPorEmpleadoMes[pago.empleadoId][clave]) {
        gananciasPorEmpleadoMes[pago.empleadoId][clave] = [];
      }
  ...
  document.getElementById('gm-count').textContent = empleados.length + ' empleado' + (empleados.length === 1 ? '' : 's');
}

function exportarGananciasMensuales() {
  const tabla = document.getElementById('gm-tabla');
  if (!tabla || !tabla.querySelector('tbody tr').textContent.includes('$')) {
    alert('No hay datos para exportar. Carga primero los datos de ganancias mensuales.');
    return;
  }
  ...
  link.setAttribute('download', 'ganancias-mensuales-' + todayIso() + '.csv');
  link.click();
}
